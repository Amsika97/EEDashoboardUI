...
@Component({
  ...
  styleUrls: [
    './metrics-report.component.css',
    '../../common/table/table-style.css',
  ],
})
export class MetricsReportComponent implements OnInit, AfterViewInit {
  label: any;
  ...
  ngOnInit(): void {
    this.service
    .httpRequest(AssessmentControllerURL.metricsReport, 'GET')
    .subscribe({
      next: (assessmentResponse: any) => {
        this.assessmentResponse1 = assessmentResponse;
        this.getMetricsReports(assessmentResponse);
      },
      error: (error) => {
        this.assessmentResponse1 === ''
        console.log(error);
      },
    });
    this.userDetailsService.userSubject.subscribe((data) => {
      this.role = data.role;
      this.getMetricsReports(this.assessmentResponse1)
    });
    ...
    this.validationService.reportsFilter.subscribe((assessmentResponse) => {
      ...
      this.reportValues = assessmentResponse.metricReportDetails;
      this.getMetricsReports(this.reportValues);
    });
  }
  ...
  deletePopUp(id: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === 'yes') {
        this.service
          .httpRequest(`/ee-dashboard/api/v1/metric/submitStatus/${id}`, 'PUT')
          .subscribe({
            next: (resp) => {
  ...
  getMetricsReports(assessmentResponse: any) {
